 ...
 * 
 * 
 * code added by:Michael Chu mchu64@bu.edu
 */
...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        else if(charNum != 0 && alreadyUsed(this.words[wordNum] + letter) == false && onSameSide(letter, lastLetter(this.words[wordNum])) == false && dictionary.hasString(this.words[wordNum] + letter)){
            return true;
        }
        else if(charNum == 0 && wordNum >= 1 && lastLetter(this.words[wordNum-1]).equals(letter) && dictionary.hasString(this.words[wordNum] + letter) && alreadyUsed(this.words[wordNum] + letter) == false){
            return true;
        }
        return false;
    }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        else if(allLettersUsed() == true && dictionary.hasFullWord(this.words[wordNum])){
            printSolution(wordNum);
            return true;
        }
        else{
        for(int val = 0; val < letters.length;val++){
            if(isValid(letters[val],wordNum,charNum)){
                this.addLetter(letters[val], wordNum);
                if(solveRB(wordNum,charNum+1,maxWords)){
                    return true;
                }
            else if(dictionary.hasFullWord(this.words[wordNum]) && words[wordNum].length() >= 3){
